function [unw_flat,unw_mask, aps, ramp, params] = deramp_dem(unw, mask, dem, deramp_method)
...
yy_mask = yy;
dem_mask = dem;
xx_mask(isnan(unw_mask)) = 0;
yy_mask(isnan(unw_mask)) = 0;
dem_mask(isnan(unw_mask)) = 0;


unw_mask2 = unw_mask;
% unw_mask2(isnan(unw_mask)) = 0;
% unw_mask2 = nonzeros(reshape(unw_mask2, x*y, 1));
unw_mask2 = reshape(unw_mask2, x*y,1);
unw_mask2(isnan(unw_mask2)) = [];
xx_mask2 = nonzeros(reshape(xx_mask, x*y, 1));
yy_mask2 = nonzeros(reshape(yy_mask, x*y, 1));
dem_mask2 = reshape(dem_mask, x*y, 1);
dem_mask2(dem_mask==0) = [];
